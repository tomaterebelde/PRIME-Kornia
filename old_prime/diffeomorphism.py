...
def scalar_field(b, n, m, coeffs, device='cpu'):
    """
...
def deform(image, params, interp='bilinear'):
    """
    ...
    u = scalar_field(b, n, cut, coeffs, device)  # [b, n,n]
    v = scalar_field(b, n, cut, coeffs, device)  # [b, n,n]
    dx = T ** 0.5 * u * n
    dy = T ** 0.5 * v * n
...
def remap(a, dx, dy, interp):
    ...
    interpolation = torch.nn.functional.grid_sample(a, torch.stack([xn, yn], dim=-1), mode=interp, padding_mode='border')
...
if __name__ == "__main__":
    ...
    diffeo = Diffeo( sT = 1., rT = 1., scut = 1., rcut = 1., cutmin = 2, cutmax = 5, alpha = 1.0)
    
    # Create checkerboard image with 4x4 squares
    checkerboard = torch.zeros(2, 3, 224, 224)
    for i in range(224):
        for j in range(224):
            if (i // 8 + j // 8) % 2 == 0:
                checkerboard[...,  i, j] = 1

    distorted_checkerboard = diffeo(checkerboard)
    plt.subplot(1,2,1)
    plt.imshow(distorted_checkerboard[0].permute(1, 2, 0))
    plt.subplot(1,2,2)
    plt.imshow(distorted_checkerboard[1].permute(1, 2, 0))
