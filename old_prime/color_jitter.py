...
class RandomSmoothColor(torch.nn.Module):
    ...
    def random_smooth_color(self, img, cut, T, freq_bandwidth=None):
        ...
        return self.apply_transform(img, {"rand_coeffs": coeff, "k": k})
    ...
    def apply_transform(self, input, params):
        ...
        coeff = params["rand_coeffs"]
        k = params["k"]

        colors = rearrange(input, "b c h w -> b c (h w)")
        freqs = torch.sin(colors[..., None] * k[None, None, None, :] * math.pi)

        transformed_colors = torch.einsum("bcf,bcnf->bcn", coeff, freqs) + colors
        # Check if its the precision and also the images
        # transformed_colors = contract("bcf, bcnf -> bcn", coeff, freqs) + colors     # This was the final implementation
       
        transformed_colors = torch.clamp(transformed_colors, 0, 1)

        transformed_image = rearrange(transformed_colors, " b c (h w) -> b c h w", **input_shape)
